...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (container) {
        ...
        if (itemCount === 1) container.classList.add("single-item");
        else if (itemCount === 2) container.classList.add("two-items");
        else if (itemCount > 1 && itemCount < 4) container.classList.add("multiple-items");
        else if (itemCount >= 4) container.classList.add("table-items");
    }
});
...
if (estadoSelectElement) {
    estadoSelectElement.addEventListener("change", function() {
        ...
        if (estadoSelecionado) carregarCidades(estadoSelecionado);
        else {
            const cidadesSelect = document.getElementById("inputCidade");
            if (cidadesSelect) cidadesSelect.innerHTML = '<option value="">Selecione a Cidade</option>' // Limpa as opções de cidades se nenhum estado for selecionado
            ;
        }
    });
...
$(document).ready(function() {
    ...
    function adjustHeader() {
        if ($(window).width() <= 480) // Checa se a largura da tela é de 480px ou menos
...
lists.forEach((list)=>{
    ...
    if (list.children.length > 1) // Itera sobre todos os <li> dentro do <ul>, começando do segundo
